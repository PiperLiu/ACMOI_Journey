...
- uu 2020.12.31 串联所有单词的子串『注意到单词是定长的、子串中单词顺序不重要，使用滑窗，left和right有点类似双指针的感觉；滑窗每次滑动长度为word.size()，因此注意起点有word.size()这么多个』 匹配 字符串 [cpp](./vsc_leetcode/30.串联所有单词的子串.cpp)